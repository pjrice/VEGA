    ...
    try:
        ...
        if not vgConfig.modelIsVoting:
            ...
            if vgConfig.noImages:
                ...
                for radius in range(vgConfig.radiusBounds[0],vgConfig.radiusBounds[1]+1):
                    vgConfig.sceneGroups[str(radius)] = vgClasses.visGroups(noImageVisicon,radius,vgConfig.collisionMethod)
            else:
                for radius in range(vgConfig.radiusBounds[0],vgConfig.radiusBounds[1]+1):
                    vgConfig.sceneGroups[str(radius)] = vgClasses.visGroups(vgConfig.currentVisicon,radius,vgConfig.collisionMethod)
        
            numberOfGroups = []
            for key in vgConfig.sceneGroups.keys():
                numberOfGroups.append(vgConfig.sceneGroups[key].groupCount)
            ...
            for uniqueGroupSet in uniqueNumGroups:
                ...
                dictKeys2Keep.append(list(vgConfig.sceneGroups.keys())[groupSetIdx])
                
            vgConfig.sceneGroupsSubset = {key: vgConfig.sceneGroups[key] for key in dictKeys2Keep}
            
            for radius in vgConfig.sceneGroupsSubset.keys():
            
                groupSlotName = "group_R"+radius
            
                vgLabeling.actr.extend_possible_slots(groupSlotName, warn=False)
            
                vgLabeling.label_groups(vgConfig.sceneGroupsSubset[radius],None)
            
                vgConfig.modVisLock = True
                for visPoint in vgConfig.sceneGroupsSubset[radius].visPoints:
                    vgLabeling.actr.modify_visicon_features([visPoint.visiconID,groupSlotName,visPoint.groupName])
                vgConfig.modVisLock = False
                
            if vgConfig.denoteGroups:
                vgLabeling.denote_group_extent(vgConfig.sceneGroupsSubset)
                
            #for key in vgConfig.sceneGroups.keys(): print(vgConfig.sceneGroups[key].groupCount)
            for key in vgConfig.sceneGroupsSubset.keys(): print(vgConfig.sceneGroupsSubset[key].groupCount)
