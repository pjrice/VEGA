...
(defmethod schedule-proc-display ((vis-mod vision-module) &optional (pdAsCommand t))
  (bt:with-recursive-lock-held ((proc-display-lock vis-mod))
    (unless (scheduled-proc-display vis-mod)
      (setf (scheduled-proc-display vis-mod) 
        (bt:with-recursive-lock-held ((param-lock vis-mod))
          (if (and (force-commands vis-mod) pdAsCommand)
...
(defun proc-display (&optional vis-mod)
  (unless vis-mod
...
(add-act-r-command "proc-display" 'proc-display "Internal vision module command for processing visual display -- do not call directly.")
...
(defun modify-visicon-features (&optional (pdAsCommand t) &rest feats)
  (let ((vis-mod (get-module :vision)))
    (when vis-mod
      (bt:with-recursive-lock-held ((param-lock vis-mod))
        (if (force-commands vis-mod)
            (dispatch-apply-list-names "modify-visicon-features" (encode-string-names feats))
          (apply 'internal-modify-visicon-features (cons vis-mod pdAsCommand feats)))))))

(defun internal-modify-visicon-features (mod &optional (pdAsCommand t) &rest feats)
  (let ((results nil)
        (updates nil))
    (when mod
      (flet ((invalid-feature (bad)
        ...
        (schedule-proc-display mod pdAsCommand)))
    results))
