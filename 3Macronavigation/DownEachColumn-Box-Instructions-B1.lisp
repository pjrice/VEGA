...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; 
;;; Filename    : DownEachColumn-Box-Instructions-B1.lisp
;;; Version     : 1
;;; 
;;; Description : Macronavigation strategy (version B: thorough strategy)
...
;;;				: * Then the model finds the race below the instructions, and start voting.
...
;;;
;;; ----- History -----
;;; 2022.02.21   Xianni Wang
;;;				: * Added productions to read instructions (line by line until screen-y reach 150 pixels) and skip left-intructions 
;;;     			: * Modified from DownEachColumn-Box-Instructions-A1.
;;;
;;; -- inherited from DownEachColumn-Box-Instructions-A1
;;; 2022.01.21   Xianni Wang
...
;;;				: * Added productions to accommodate 4 ballot layouts (ballot_instructions_1,2,3,4; both top-instr & left-instr)
;;; 2021.11.04   Xianni Wang
;;;				: * Modified productions to do random visual search in header, footer & instructions boxes
;;;				: * Added productions to handle bubbles in the instructions
;;; 2021.10.27   Xianni Wang
;;;				: * Modified productions to do random visual search in headers & instructions boxes
...
;;;				: * Modified visual location buffers to make the model navigate among races & columns
;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;
;;; General Docs:
...
;attend current instr box, then navigate to footer
...
; if footer is found, attend footer box
(P Attend-Footer-Box
...
=goal>
	state		read-left-instr
	
)

; ***** read left-instructions (until screen-y is greater than 150)
; find the top-leftmost text in the instructions
(P Read-Current-Left-Instructions
...
=goal>
	state		 read-left-instr-2
	
)

;find the text to the right (process instructions row by row)
(P Read-Current-Left-Instructions-2
...
=goal>
	state		attend-left-instr
	
)

(P Attend-Current-Left-Instructions-Text
=goal>
	state     attend-left-instr
...
=goal>
	state        read-left-instr-3

)

(P Read-Current-Left-Instructions-3
...
=goal>
	state          attend-left-instr-2
)

(P Attend-Current-Left-Instructions-Text2
=goal>
	state     attend-left-instr-2
...
; if reach the end of the row, navigvate to the next line

(P Read-Current-Left-InstructionsRow-Failure
...
=goal>
	state		  find-text-next-row

)

(P Read-Current-Left-InstructionsRow-Failure2
...
; find the text right below
(P Find-Left-Instructions-NextRow
...
; if left-instruction is long (screen-y >=150), then skip the rest of instructions, and move to races in the first column

(P Read-Current-Left-Instructions-Failure
...
;**********************************************************
;**********************************************************
; attend current instr box, and then move to races
...
=goal>
	state		attending-first-racebox

)

(P Attend-First-RaceBox
...
;if no buttons found in current racebox, find current box, then navigate to the next race
