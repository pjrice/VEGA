...
def proc_display_monitor(cmd,params,success,results):
    ...
    try:
        ...
        for radius in range(radiusBounds[0],radiusBounds[1]+1):
            sceneGroups[str(radius)] = visGroups(currentVisicon,radius,'box')
        
            
        numberOfGroups = []
        for key in sceneGroups.keys():
            numberOfGroups.append(sceneGroups[key].groupCount)
            
            
        uniqueNumGroups = list(np.unique(np.array(numberOfGroups)))
        
        dictKeys2Keep = []
        for uniqueGroupSet in uniqueNumGroups:
            
            groupSetIdx = numberOfGroups.index(uniqueGroupSet)
            dictKeys2Keep.append(list(sceneGroups.keys())[groupSetIdx])
            
        
        sceneGroupsSubset = {key: sceneGroups[key] for key in dictKeys2Keep}
        
        for radius in sceneGroupsSubset.keys():
            
            groupSlotName = "group_R"+radius
            
            actr.extend_possible_slots(groupSlotName, warn=False)
            
            label_groups(sceneGroupsSubset[radius],None)
            
            for visPoint in sceneGroupsSubset[radius].visPoints:
                actr.set_chunk_slot_value(visPoint.visiconID,groupSlotName,visPoint.groupName)
            
            
            
        #for key in sceneGroups.keys(): print(sceneGroups[key].groupCount)
