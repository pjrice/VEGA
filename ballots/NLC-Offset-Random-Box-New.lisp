...
;;; Version     : 1
;;; 
;;; Description : A ballot with image background
...
;;; 
;;; ----- History -----
;;; 2020.02.25  Xianni Wang
;;;				: * add Dan's code to handle image     
			...
			(loop 	
				...
				(let* (
					...
					(setf current-column-sizes (append current-column-sizes (list (+ (list-length candidates) 1))))
                                        ...
                                        (add-image-to-exp-window window "box" "lb.gif" :x (- current-x 4) :y current-y :width 280 :height (+ y-spacing-after-title (* (- (list-length candidates) 1) y-spacing-between-candidates) (* (list-length (list (+ (list-length candidates) 1))) 14) 12))
                                        ...
                                        ;(make-instance 'image-vdi :file "lb.gif" :dialog-item-text "box" :x-pos (- current-x 4) :y-pos current-y :width 280 :height (+ y-spacing-after-title (* (- (list-length candidates) 1) y-spacing-between-candidates) (* (list-length (list (+ (list-length candidates) 1))) 14) 12) ) )

					;(length (cand-lst race))
