...
;   (instr-c :accessor instr-c :initarg :instr-c :initform nil)
   ...
   (nonrace-a :accessor nonrace-a :initarg :nonrace-a :initform nil)
   (nonrace-b :accessor nonrace-b :initarg :nonrace-b :initform nil)
   ))
...
the line."
                ...
                :nonrace-a "By casting this ballot, I do
pledge myself to abide by the
results of this Primary 
Election."

                :nonrace-b "Continue voting
next side"  
...
(defun vote (realtime use-model visible dolog &optional (contest-parameters '(30 35 18)))
    ...
    (setf *window* (open-exp-window "Ballot" :visible t :x 0 :y 0 :width 710 :height 740))
    ...
    (let* (
        ...
        ; add image box for instructions
        (add-image-to-exp-window *window* "box" "lb.gif" :x (- instr-x 4) :y instr-y :width 225 :height 300 :clickable nil)
        ...
        ; add image box
        (add-image-to-exp-window *window* "box" "lb.gif" :x (- instr-x 4) :y 350 :width 225 :height 50 :clickable nil)
        
        ;add non-race texts    
        (add-text-to-exp-window *window* (nonrace-a instr) :color 'black :x (- instr-x 4) :y 352)
        
        ; Place non-race in the middle column
        ; add image box
        (add-image-to-exp-window *window* "box" "lb.gif" :x 242 :y (- wheight 78) :width 225 :height 30 :clickable nil)
        
        ;add non-race texts    
        (add-text-to-exp-window *window* (nonrace-b instr) :color 'black :x 265 :y (- wheight 78))
        
        ; Place non-race in the right column
        ; add image box
        (add-image-to-exp-window *window* "box" "lb.gif" :x 478 :y (- wheight 78) :width 225 :height 30 :clickable nil)
        
        ;add non-race texts    
        (add-text-to-exp-window *window* (nonrace-b instr) :color 'black :x (+ instr-x 500) :y (- wheight 78))
