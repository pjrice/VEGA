...
; Winsconsin Eror End
	   ...
	   (make-instance 'contest
		 :office-name "UnitedStatesRepresentative7"
		 :cand-lst
		 (list
		  (make-instance 'cand-choice
...
(defun vote (realtime use-model visible dolog &optional (contest-parameters '(35 35 18)))
    ...
    (setf *window* (open-exp-window "Ballot" :visible t :x 0 :y 0 :width 770 :height 720))
    ...
    (let* (
        ...
        (add-image-to-exp-window *window* "box" "lb.gif" :x (- instr-x 4) :y instr-y :width 225 :height 550 :clickable nil)
