...
(defun vote (realtime use-model visible dolog &optional (contest-parameters '(30 35 18)))
    ...
    (let* (
        ...
        ; add image box for instructions
        (add-image-to-exp-window *window* "box" "lb.gif" :x (- instr-x 4) :y instr-y :width 225 :height 470 :clickable nil)
        ...
        (add-text-to-exp-window *window* (instr-d instr) :color 'black :x instr-x :y (+ instr-y 282))
        
        ; Displays buttons & image
		(add-button-to-exp-window *window* :color 'white :text "" :x instr-x :y (+ instr-y 400) :width 20 :height 10)
