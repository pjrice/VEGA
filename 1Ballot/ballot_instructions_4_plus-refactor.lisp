...
(defun vote (realtime use-model visible dolog &optional (contest-parameters '(30 35 18)))
    ...
    (setf *window* (open-exp-window "Ballot" :visible t :x 0 :y 0 :width 710 :height 760))
    ...
    (let* (
        ...
        ; Place non-race content in the left column
        ; add image box
        (add-image-to-exp-window *window* "box" "lb.gif" :x (- instr-x 4) :y 175 :width 225 :height 50 :clickable nil)
        
        ;add non-race texts    
        (add-text-to-exp-window *window* (nonrace-a instr) :color 'black :x (- instr-x 1) :y 175)
        
        ; add image box
        (add-image-to-exp-window *window* "box" "lb.gif" :x (- instr-x 4) :y (- wheight 78) :width 225 :height 30 :clickable nil)
        
        ;add non-race texts    
        (add-text-to-exp-window *window* (nonrace-b instr) :color 'black :x (- instr-x 1) :y (- wheight 78))
        
        ; Place non-race content in the right column
        ; add image box
