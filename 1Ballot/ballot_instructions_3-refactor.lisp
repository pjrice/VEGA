...
PLEASE TURN BALLOT OVER."
                ...
                :instr-a "NOTICE TO ELECTORS: This ballot may be invalid unless initialed by 2 
election inspectors. If cast as an absentee ballot, the ballot must 
bear the initials of the manucipal clerk or deputy clerk. To vote 
for the candidate of your choice, blacken the oval to the left of 
the candidate's name."
; To vote for a person whose name does not 
; appear on the ballot, write the person's name on the line provided and blaken the oval to the left of 
; the line."
                :instr-d "IMPORTANT: Blacken the oval to the LEFT of the name of the candidate.
When voting for governor and lieutenant governor, you may vote only
for the candidate on ticket jointly."        
    ))
...
(defun vote (realtime use-model visible dolog &optional (contest-parameters '(30 35 18)))
    ...
    (let* (
        ...
        (add-image-to-exp-window *window* "box" "lb.gif" :x (- instr-x 6) :y instr-y-header :width 720 :height 13 :clickable nil)
        ...
        (add-text-to-exp-window *window* (instr-d instr) :color 'black :x instr-x :y (+ instr-y 88))
        
        ; Displays buttons & image
		(add-button-to-exp-window *window* :color 'white :text "" :x (+ instr-x 540) :y (+ instr-y 30) :width 20 :height 10)
		(add-button-to-exp-window *window* :color 'white :text "" :x (+ instr-x 540) :y (+ instr-y 48) :width 20 :height 10)
        (add-button-to-exp-window *window* :color 'white :text "" :x (+ instr-x 540) :y (+ instr-y 66) :width 20 :height 10)
        
        (add-image-to-exp-window *window* "box" "pen.gif" :x (+ instr-x 570) :y (+ instr-y 20) :width 100 :height 70 :clickable nil)
